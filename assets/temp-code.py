...
def create_faiss_index(all_chunks, current_hash, force_rebuild=False):
    ...
    if os.path.exists(index_file) and not force_rebuild:
        with open(index_file, 'rb') as f:
            ...
            if saved_data['hash'] == current_hash:
                print("Loaded existing FAISS index.")
                return saved_data['index']
            else:
                print("Input documents have changed. Rebuilding index.")
    ...
    with open(index_file, 'wb') as f:
        pickle.dump({'index': index, 'hash': current_hash}, f)
...
def main():
    ...
    index = create_faiss_index(all_chunks, current_hash, force_rebuild=False)# rebuild the index when the source documents change by setting force_rebuild=True
    # I have provided default questions to1 guide users and demonstrate system capabilities.
    default_questions = [
        "Select a question",
    ...
    if st.button("Get Answer"):
        if user_query and user_query != "Select a question":
            with st.spinner("Generating answer..."):
                ...
                all_chunks, chunk_to_doc, current_hash = process_pdfs(current_hash)
                index = create_faiss_index(all_chunks, current_hash)
